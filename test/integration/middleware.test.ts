...
const runtimes: ["Node" | "Deno" | "Bun", "Node" | "Web"][] = [];

if ("Deno" in globalThis) {
  runtimes.push(["Deno", "Node"]);
  runtimes.push(["Deno", "Web"]);
} else if ("Bun" in globalThis) {
  runtimes.push(["Bun", "Node"]);
  runtimes.push(["Bun", "Web"]);
} else {
  runtimes.push(["Node", "Node"]);
}

runtimes.forEach((runtimeCase) => {
  const [runtime, target] = runtimeCase;
  describe(`create${target}Middleware - ${runtime}`, () => {
    ...
    it("Handles missing headers", async () => {
      ...
      assert(response.status === 400);
      assert(
        /Required headers missing: x-github-event/.test(await response.text()),
      );
